...
app.get("/todos/search", verifyToken, async (req, res) => {
    try {
        ...
        // Handle empty or undefined search - return all todos
        if (!search || search.trim() === '') {
            res.status(200).json({ todos: user.todos });
            console.log("All todos fetched for user:", req.user.username);
            return;
        }
        ...
        console.log(`Search "${search}" returned ${filteredTodos.length} todos for user:`, req.user.username);
    } catch (error) {
